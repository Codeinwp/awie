...
/**
 * Recent posts type F widget
 *
 */

class Awye_Recent_F extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'recent_posts_c recent_posts_f clearfix', 'description' => __( 'Recent posts widget Type F - one category', 'awie') );
		parent::__construct('recent_posts_f', __('Awye: Recent posts type F', 'awie'), $widget_ops);
		$this->alt_option_name = 'recent_posts_f';
	...
	public function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'recent_posts_f', 'widget' );
		}
		...
		$secound_query = new WP_Query( array(
			'posts_per_page'      => 4,
			'no_found_rows'       => true,
			'post_status'         => 'publish',
			'ignore_sticky_posts' => true,
...
<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'recent_posts_f', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['recent_posts_f']) )
			delete_option('recent_posts_f');

		return $instance;
	}

	public function flush_widget_cache() {
		wp_cache_delete('recent_posts_f', 'widget');
	}
...
?>
		...
		<p><label for="<?php echo $this->get_field_id( 'cat_one' ); ?>"><?php _e( 'Enter the slug for your first category.', 'awie' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'bg_color' ); ?>" style="display:block;"><?php _e( 'Background color', 'awie' ); ?></label> 
		...
		<p><label for="<?php echo $this->get_field_id( 'text_color' ); ?>" style="display:block;"><?php _e( 'Text color', 'awie' ); ?></label> 
